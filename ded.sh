...
HOME_DIR=`cd ~ && pwd -P`
LOCAL_REPO="$HOME_DIR/.m2/repository"
REMOTE_REPO="jcenter"
#0:No handle; 
...
if [ ! -d "$LOCAL_REPO" ]; then
    echo "There local repository directory is not exit"
	echo "we now create a directory with the path $REPO_PATH"
	mkdir -p $REPO_PATH
fi
...
{
    echo "---------------------------List all result in $LOCAL_REPO:-------------------------"
    python $PROGDIR/py/local_dependency.py $LOCAL_REPO $DOWNLOAD_JAR
}
